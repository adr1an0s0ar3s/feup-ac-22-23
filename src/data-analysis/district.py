...
def plot_loan_by_region(df):
    ...
    region_df["status"] = (
        region_df["status"] + 1
    ) / 2  # Transform status from -1/1 to 0/1 to improve plot
    region_df = (
        region_df.groupby("region")
        .agg(
            count=("region", "count"),
            mean_status=("status", "mean"),
            median_status=("status", "median"),
        )
        .reset_index()
    )
    region_df = region_df.sort_values(by=["count"], ascending=False).reset_index(
        drop=True
    )
    ...
    sns.barplot(
        data=region_df.sort_values(by="region"),
        x="region",
        y="mean_status_percent",
        ax=ax[0],
    )
...
def plot_loan_by_district(df):
    district_df = df.copy()
    district_df["status"] = (
        district_df["status"] + 1
    ) / 2  # Transform status from -1/1 to 0/1 to improve plot
    district_df = (
        district_df.groupby("districtName")
        .agg(
            count=("districtName", "count"),
            mean_status=("status", "mean"),
            median_status=("status", "median"),
        )
        .reset_index()
    )
    district_df = district_df.sort_values(by=["count"], ascending=False).reset_index(
        drop=True
    )
    ...
    sns.barplot(y="mean_status", x="districtName", data=district_df, alpha=0.5, ax=ax2)
