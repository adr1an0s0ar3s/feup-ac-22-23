...
def main():
    ...
    while test_size <= max_test_size:
        X_train, X_test, y_train, y_test = train_test_split(
            X, y, test_size=test_size, random_state=42, shuffle=False, stratify=None
        )
        print(
            f"test_size={test_size:01f}: train={y_train.mean():02f} - test={y_test.mean():02f} -> diff={y_train.mean()-y_test.mean():02f} | (Target: {y.mean()})"
        )
        test_size += 0.01
